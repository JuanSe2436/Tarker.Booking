...
namespace Tarker.Booking.Api.Controllers
{
    ...
    public class CustomerController : ControllerBase
    {
            ...
            [FromServices] IValidator<UpdateCustomerModel> validator)
        {
            ...
            var data = await updateCustomerCommand.Excute(model);
        ...
        public async Task<IActionResult> Delete(
            int customerId,
            [FromServices] IDeleteCustomeCommand deleteCustomeCommand)
        {
            if (customerId == 0)
                return StatusCode(StatusCodes.Status400BadRequest, ResponseApiService.Response(StatusCodes.Status400BadRequest));
            var data = await deleteCustomeCommand.Execute(customerId);
            if(!data)
            ...
            [FromServices] IGetCustomerByIdQuery getCustomerByIdQuery)
        {
            if (userId == 0)
                return StatusCode(StatusCodes.Status400BadRequest, ResponseApiService.Response(StatusCodes.Status400BadRequest));
            var data = await getCustomerByIdQuery.Execute(userId);
            if (data == null)
                return StatusCode(StatusCodes.Status404NotFound, ResponseApiService.Response(StatusCodes.Status404NotFound));
            return StatusCode(StatusCodes.Status200OK, ResponseApiService.Response(StatusCodes.Status200OK, data));
